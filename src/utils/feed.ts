...
export function generateFeed(posts: PostSummaryDto[]) {
  const feed = new Feed({
    ...
    image: encodeURIComponent(
      `https://og-image.markey-dev.com/${TITLE}.jpeg?theme=light&md=1&fontSize=100px&images=https%3A%2F%2Favatars.githubusercontent.com%2Fu%2F34389937`
    ),
  ...
  posts.forEach(post => {
    ...
    feed.addItem({
      ...
      // FIXME:encodeURIComponentを使うとfeed.rss2()でエラーになる（謎）
